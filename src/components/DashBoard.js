...
const DashBoard = () => {
  ...
  const loadParmetersPlan = async (uid) => {
    ...
    data.append("uid", uid);
    const url =
      "http://localhost:8012/www/tripmasterv01/public/loadtodashboard.php";
    try {
  ...
  const changeDB = async (idDB, uid) => {
    ...
    data.append("uid", uid);
    data.append("id", idDB);
    const url = "http://localhost:8012/www/tripmasterv01/public/DeletPlan.php";
    try {
      const res = await axios.post(url, data);
      if (res.data) {
  ...
  const updateDashboardInDB = async (updatedPlan) => {
    ...
    const url =
      "http://localhost:8012/www/tripmasterv01/public/UpdateDashboardPlan.php";
    try {
      const res = await axios.post(url, data);
    } catch (err) {}
  };
